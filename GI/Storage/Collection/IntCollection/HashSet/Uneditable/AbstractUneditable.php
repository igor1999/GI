 ...
 */
namespace GI\Storage\Collection\IntCollection\HashSet\Uneditable;

use GI\Storage\Collection\IntCollection\HashSet\Immutable\Immutable;
...
abstract class AbstractUneditable extends Immutable implements UneditableInterface
{
     ...
     * @throws \Exception
     */
    protected function edit(string $key, int $item)
    {
        if (!$this->has($key)) {
    ...
    public function __call(string $method, array $arguments = [])
    {
        ...
        if (!empty($set) && !empty($value)) {
            ...
            $result = $this->edit($set, (int)$value);
        } elseif (!empty($set)) {
            if (empty($arguments)) {
